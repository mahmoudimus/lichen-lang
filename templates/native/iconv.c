...
__attr __fn_native_iconv_iconv(__attr __args[])
{
    ...
    __fragment *f = __load_via_object(state->value, __ATTRPOS(__data__)).seqvalue;

    /* Obtain the string, start position, and remaining bytes from the state. */

    char *inbuf = __load_via_object(f->attrs[0].value, __ATTRPOS(__data__)).strvalue;
    int start = __load_via_object(f->attrs[1].value, __ATTRPOS(__data__)).intvalue;
    int remaining = __load_via_object(f->attrs[2].value, __ATTRPOS(__data__)).intvalue;
...
__attr __fn_native_iconv_iconv_open(__attr __args[])
{
    ...
    char *t = __load_via_object(tocode->value, __ATTRPOS(__data__)).strvalue;
    /* fromcode.__data__ interpreted as string */
    char *f = __load_via_object(fromcode->value, __ATTRPOS(__data__)).strvalue;
    iconv_t result;
    __attr attr;
