...
class CommonModule:
    ...
    def convert_ifexp_node(self, n):
        ...
        """
        Convert the given if expression node 'n'. An if expression is considered
        as mapping to a function body containing an if statement as follows:

        <expr> if <test> else <altexpr>
        ...
        The <argnames> are populated with defaults after the node has been
        processed.
        """

        node = compiler.ast.Lambda(
            [], [], 0,
            compiler.ast.If([
                (n.test, compiler.ast.Return(n.then))
                ],
                compiler.ast.Return(n.else_)
                ))

        return node

    def convert_listcomp_node(self, n):

        """
        Convert the given list comprehension node 'n'. A list comprehension is
        considered as mapping to a function body containing a for loop as
        follows:

        [<expr> for <varexpr1> in <list1> if <ifexpr1> for <varexpr2> in <list2> if <ifexpr2> if <ifexpr3>]

        lambda <argnames>:
            <result> = []
            for <varexpr1> in <list1>:
                if <ifexpr1>:
                    for <varexpr2> in <list2>:
                        if <ifexpr2>:
                            if <ifexpr3>:
                                <result>.append(<expr>)
            return <result>
        ...
        node = compiler.ast.Lambda(
            ...
            compiler.ast.Assign([compiler.ast.AssName(temp, "OP_ASSIGN")],
                compiler.ast.List([])
                ),

            # for ...

            self.convert_listcomp_for_node(n.quals[0], n.quals[1:], n.expr, temp),

            # return <result>

            compiler.ast.Return(compiler.ast.Name(temp))
            ]))

        return node

    def convert_listcomp_for_node(self, loop, following_loops, expr, temp):

        """
        Return a node representing 'loop', encapsulating 'following_loops' and
        employing 'expr' in the innermost loop body appending to 'temp'.
        """

        if loop.ifs:
            body = self.convert_listcomp_if_node(loop.ifs[0], loop.ifs[1:], following_loops, expr, temp)
        elif following_loops:
            body = self.convert_listcomp_for_node(following_loops[0], following_loops[1:], expr, temp)
        else:
            body = self.convert_listcomp_body_node(expr, temp)

        return compiler.ast.For(loop.assign, loop.list, compiler.ast.Stmt([body]), None)

    def convert_listcomp_if_node(self, if_, following_ifs, following_loops, expr, temp):

        """
        Return a node representing 'if_', encapsulating the 'following_ifs' and
        'following_loops' and employing 'expr' in the innermost loop body
        appending to 'temp'.
        """

        if following_ifs:
            body = self.convert_listcomp_if_node(following_ifs[0], following_ifs[1:], following_loops, expr, temp)
        elif following_loops:
            body = self.convert_listcomp_for_node(following_loops[0], following_loops[1:], expr, temp)
        else:
            body = self.convert_listcomp_body_node(expr, temp)

        return compiler.ast.If([(if_.test, compiler.ast.Stmt([body]))], None)

    def convert_listcomp_body_node(self, expr, temp):

        "Return a node appending 'expr' to 'temp'."
